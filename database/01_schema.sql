...
-- Consolidated schema (all migration_add_* DDL merged; seed scripts remain separate)
-- Run order: 00_drop_all.sql -> 01_schema.sql -> 02_seed_master_data.sql -> 03/04 seeds
...
-- Scope categories (emission scopes 1–4; aligned with ideacarb-client-app)
...
-- Fuel resources (emission factors per scope category)
...
-- Template activity groups (per-industry activity groups)
...
-- App usage tracking (telemetry from org-app instances)
...
CREATE UNIQUE INDEX idx_template_activity_groups_template_legacy ON template_activity_groups(template_id, legacy_activity_group_id);
CREATE INDEX idx_template_activity_groups_template_sort ON template_activity_groups(template_id, sort_order);
CREATE INDEX idx_template_activity_groups_scope_category ON template_activity_groups(scope, scope_category_id);
CREATE INDEX idx_template_activity_groups_deleted_at ON template_activity_groups(deleted_at) WHERE deleted_at IS NULL;

-- Activity group fuel resource indexes
