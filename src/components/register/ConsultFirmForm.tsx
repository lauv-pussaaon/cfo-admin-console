...
const firmSchema = z.object({
  ...consultAccountSchema,
  firmName: z.string().min(1, 'กรุณากรอกชื่อบริษัทที่ปรึกษา').max(200),
})
...
export default function ConsultFirmForm ({ onSuccess }: { onSuccess: () => void }) {
      ...
      <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={consultFormGridSx}>
        <TextField
          ...
          InputProps={{
            ...
            endAdornment: endCheck(fieldIsValid(values.firmName, errors.firmName)),
          }}
        />
        ...
        <FormControl fullWidth error={!!errors.industries} sx={{ gridColumn: '1 / -1' }}>
          <InputLabel id="firm-register-industries-label">อุตสาหกรรม</InputLabel>
          <Select
            labelId="firm-register-industries-label"
            multiple
